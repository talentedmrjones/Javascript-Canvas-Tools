...
4)p=g.Blends[a](b[i],b[i+1],b[i+2],b[i+3],c[i],c[i+1],c[i+2],c[i+3],d),c[i]=p[0],c[i+1]=p[1],c[i+2]=p[2],c[i+3]=p[3];this.setImageData(f);"function"==typeof e.post&&e.post.apply(this);return this};g.Canvas.prototype.setCanvas=function(a){switch(!0){case typeof a=="string"&&a.length>0:a=document.getElementById(a);break;default:a=document.createElement("canvas")}if(!0===a instanceof HTMLImageElement){var b=document.createElement("canvas");b.width=a.width;b.height=a.height;b.getContext("2d").drawImage(a,
0,0);a=b}if(!1===a instanceof HTMLCanvasElement)throw Error("canvas is required as HTMLCanvasElement, HTMLImageElement, or a string representing the id of a canvas or img element.");this.canvas=a;this.context=this.canvas.getContext("2d");return this};g.Canvas.prototype.setImageData=function(a){this.context.putImageData(a,0,0)};g.Canvas.prototype.getPNG=function(){return this.canvas.toDataURL("image/png")};g.Canvas.prototype.setDimensions=function(a,b){this.canvas.width=a;this.canvas.height=b;return this};
g.Canvas.prototype.getWidth=function(){return this.canvas.width};g.Canvas.prototype.getHeight=function(){return this.canvas.height};if("CanvasTools"in window)throw Error("'CanvasTools' is already defined in the window object.");else window.CanvasTools=g})();